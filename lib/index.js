                            ...
                            if( TmpSep2 != "" )
                            {
                                ...
                                if( TmpSep2 != "" )
                                {
                                    ...
                                    if( TmpSep3A == "AccountName" )
                                    {
                                        ...
                                        if( RetFinal.hasOwnProperty(TmpSep3B) == false )
                                        {

                                            RetFinal[TmpSep3B]                      = {};
                                            RetFinal[TmpSep3B]["Conta"]             = TmpSep3B;
                                            RetFinal[TmpSep3B]["Status"]            = 0; 

                                            try {

                                                RetFinal[TmpSep3B]["Status"]            = await contas_status_retornar(TmpSep3B);
...
async function contas_status_retornar(Conta) {

    return new Promise((resolve, reject) => {

        try {

            ObjPs.addCommand('Get-MailEnablePostoffice -Postoffice "' + Conta + '" -Setting poEnabled')
            ObjPs.invoke()
            .then(output => {
                ...
                if( output != "" )
                {
                    ...
                    var RetOutput       = output.trim();
                    
                    resolve(RetOutput);

                }else{

                    throw "Falha em Executar Comando";
